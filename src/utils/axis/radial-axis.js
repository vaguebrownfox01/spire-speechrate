...
function radialAxis(angleScale, startRadius, endRadius, outer) {
	...
	function axis(context) {
		...
		var values =
			...
			anglePos = (angleScale.bandwidth ? center : identity)(
				angleScale.copy()
			),
			...
			return (
				...
				(Math.abs(endAngle - startAngle) >= 2 * Math.PI // Full-circle
					? "A" +
					  ...
					  "A" +
					  [r, r, 0, 1, 1]
							.concat(polar2cart(startAngle, r))
							.join(",")
					: "") +
				"A" +
				[
					...
					r,
					0,
					Math.abs(endAngle - startAngle) % (2 * Math.PI) > Math.PI
						? 1
						: 0, // Large arc flag
		...
		line.attr("x1", 0)
			...
			.attr("x2", function (d) {
				return (
					polar2cart(anglePos(d), tickSizeInner)[0] * (outer ? 1 : -1)
				);
			})
			.attr("y2", function (d) {
				return (
					polar2cart(anglePos(d), tickSizeInner)[1] * (outer ? 1 : -1)
				);
			});
	...
	axis.tickArguments = function (_) {
		return arguments.length
			? ((tickArguments = _ == null ? [] : Array.prototype.slice.call(_)),
			  axis)
			: tickArguments.slice();
	};
	...
	axis.tickValues = function (_) {
		return arguments.length
			? ((tickValues = _ == null ? null : Array.prototype.slice.call(_)),
			  axis)
